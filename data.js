...
// 0 = male; 1 = female; 2 = singular they
...
// This function is used to retrieve all settings to reduce loading time
function getallsett() {
    var db = getDatabase();
    var res="";
    db.transaction(function(tx) {
        var rs = tx.executeSql('SELECT uid, value FROM settings ORDER BY uid ASC;');
        if (rs.rows.length > 0) {
            res = rs.rows
        } else {
            res = -1;
