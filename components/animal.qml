...
Image {
    ...
    Image {
        id: shadow
        source: "qrc:///img/moose_shadow.png"
    ...
    Text{
        id: name
        visible: false
        x: animal.maxwidth + 3*page.scale
    ...
    function tick(){
        ...
        if(age < 19*400){
            ...
            animal.height = Math.round(factor * maxwidth);
            animal.width = Math.round(factor * maxwidth);
        }
        else{
            animal.width = maxwidth;
            animal.height = maxwidth;
        }
        ...
        if(moving){
            ...
            else if(x > rect.width - animal.width){
                x = rect.width - animal.width;
                randomdirection();
            }

            // Keep animal on screen (y axis)
            if(absy < page.menuheight){
                absy = page.menuheight;
                randomdirection();
            }
            else if(absy > rect.height - animal.height){
                absy = rect.height - animal.height;
                randomdirection();
            }
        ...
        // Scale jumping animation with screen size (ugly code, sorry)
        yshift = Math.round(yshift*page.scale);
    ...
    function importfromdna(dna){
        ...
        if(dna.length === 40){
            ...
            var basepath = 'qrc:///img/moose';

            var color = parseInt(dna.substr(2, 2), 2) + 1;
            source = basepath + color + '.png';
            viewarea = (70 + parseInt(dna.substr(4, 3), 2) * 15)*page.scale; // Scale with animal width to ensure consistent gameplay on different screens
            ...
            minspeed = (0.5 + parseInt(dna.substr(27, 3), 2)/2)*page.scale; // Scale with animal width to ensure consistent gameplay on different screens
            maxspeed = (minspeed + parseInt(dna.substr(30, 3), 2)/1.5)*page.scale // Scale with animal width to ensure consistent gameplay on different screens
