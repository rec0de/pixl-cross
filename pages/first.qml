...
Rectangle{
    ...
    property int menuheight: menu.height // Read by moose component for collision detection

    // Emulates Sailfish Theme vars
    Item{
        id: theme
    ...
    Component.onCompleted: {
        ...
        if(DB.getsett(5) < 4 || DB.getsett(5) === -1){
            DB.setsett(5, 4);
            DB.setsett(0, 0); // Set day mode
            page.firststart = true;
            welcome.visible = true;
            log('firststart', false, false, false, false)
        }
    ...
    function about() {
        pageStack.push(Qt.resolvedUrl("qrc:///pages/about.qml"))
        pause();
    }

    // Pause game and go to help page
    function help() {
        pageStack.push(Qt.resolvedUrl("qrc:///pages/help.qml"))
        pause();
    }
        ...
        if(ticker.running == false){
            ...
            for(var i = 0; i < page.animals.length; i++){
                ...
                if(page.animals[i].name === 'Eli'){
                    ...
                    page.animals[i].source = 'qrc:///img/eegg' + color + '.png';
                }
                else{
                    color = parseInt(page.animals[i].dna.substr(2, 2), 2) + 1;
                    page.animals[i].source = 'qrc:///img/moose' + color + '.png';
                }
            }
    ...
    function cleanup(){
        ...
        // Log story message randomly (1 in 4 chance every 60 seconds)
        // Average time for complete story: (4*60*22)/60 = ~ 88min = ~ 1.5h
        if((page.playtime % 80) === 0 && Math.floor(Math.random()*4)==1){
    ...
    function spawnpredator(){
        if(page.spawnpred){
            var animal_comp = Qt.createComponent("qrc:///components/predator.qml");
    ...
    Image {
        id: background
        source: (page.scale === 1 ? "qrc:///img/back_trans.png" : (page.scale === 1.5 ? "qrc:///img/back_trans_15.png" : (page.scale === 2 ? "qrc:///img/back_trans_20.png" : "qrc:///img/back_trans_25.png")))
        opacity: 1
        width: rect.width
        height: rect.height
        fillMode: Image.Tile
    ...
    Image {
        ...
        smooth: false
        id: tree
        visible: page.height > (500 + height - 9*page.scale) ? true : false
        opacity: 1
        width: 80 * page.scale
    ...
    Image {
        source: "qrc:///img/tree_shadow.png"
        smooth: false
        id: treeshadow
        visible: page.height > (500 + tree.height - 9*page.scale) ? true : false
        opacity: 1
        width: 80 * page.scale
        height: 20 * page.scale
        x: tree.x
        y: tree.y + tree.height - 20*page.scale
        z: 0
    }
    ...
    Rectangle{
        ...
        width: (rect.width > Math.round(170 * Screen.pixelDensity)) ? Math.round(170 * Screen.pixelDensity) : rect.width
        border.color: '#ffffff'
        border.width: 5
        MouseArea {
            anchors.fill: parent
        ...
        Label {
            id: welcometext
            visible: parent.visible
            text: 'Welcome to pixl. Tap anywhere to place food. Enjoy!'
            font.pointSize: 21
            color: '#ffffff'
            font.family: pixels.name
            lineHeight: 1.1
    ...
    Rectangle{
        ...
        Label {
            id: sorrytext
            visible: parent.visible
            text: 'Sorry! For some reason, your system does not support the database system used by pixl. Unfortunately, pixl won\'t run properly on your device.'
            font.pointSize: 21
            color: '#ffffff'
            font.family: pixels.name
            lineHeight: 1.1
    ...
    Rectangle{
        ...
        z: 90000
        color: rect.color
        height: animalstats_col.height * 1.2
        width: rect.width
        ...
        MouseArea {
            anchors.fill: parent
            onClicked: pageStack.push(Qt.resolvedUrl("qrc:///pages/aboutanimal.qml"), {name: parent.a_name, dna: parent.a_dna, age: parent.a_age, local: parent.a_local, id: parent.a_id});
        }
    ...
    Rectangle{
        ...
        y: rect.height - animalstats.height - 5
        height: 5
        width: parent.width
        color: '#ffffff'
    }
